...
class TestAdvanceGlobal:
    ...
    def test_missing_config_falls_back_gracefully(self, override_dir, config_path):
        # No config file on disk -- must not raise, expires_at falls back to
        # now + 5h via resolve_expires_at's own fallback path.
...
class TestStatusGlobal:
    ...
    def test_status_after_advance_reports_rung_and_expiry(self, override_dir):
        usage_override_cli.advance_global(override_dir=override_dir)
        status = usage_override_cli.status_global(override_dir=override_dir)
        assert status["rung"] == 0.90
        assert status["expires_at"] is not None

    def test_status_unlimited_rung_reported_as_unlimited(self, override_dir):
        usage_override_cli.advance_global(override_dir=override_dir)
        usage_override_cli.advance_global(override_dir=override_dir)
        usage_override_cli.advance_global(override_dir=override_dir)
        status = usage_override_cli.status_global(override_dir=override_dir)
