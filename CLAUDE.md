...
`build_stop_hook_settings_file()` also injects a no-matcher PreToolUse hook (governed by `config.context_pause`, on by default) that blocks a worker's tool calls once its context usage crosses an effective limit, except for `Write`/`Read`/`TodoWrite` — enough to write a handoff and end its turn. The effective limit is `min(threshold * window, max_tokens)` — a flat 75% threshold of a 1M-token window would be 750K tokens, far past the point a worker can still usefully write a handoff, so `max_tokens` (default `250_000`) caps the absolute token count regardless of window size. On the default 1M window this yields a 250K-token pause point; on Haiku's smaller 200K window the 75% fraction (150K) stays under the cap and controls instead. The 1M window default matches current Opus/Sonnet/Fable models (2026-07 catalog); the hook script detects a Haiku model id in the transcript (case-insensitive substring match) and caps the effective window at Haiku 4.5's real 200K there instead — no full model map. The hook script itself is generated by `context_pause_hook.py` (stdlib-only, self-contained — it must not import `maniple_mcp` since it runs standalone in the worker's shell), is invoked as `python3 context_pause_hook.py <threshold> <window_tokens> <max_tokens>`, and fails open on any error. **Codex workers are excluded** — no hook mechanism exists for Codex.
